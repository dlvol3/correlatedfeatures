...
for fi in range(len(fs_sb)):
    ...
    # training RF
    # training.shape[1]
    x = training.iloc[:, 0:(training.shape[1] - 1)].values  # Features for training
    ...
    plt.show()
    # %%
    featurelist = training.iloc[:, 0:(training.shape[1] - 1)].columns.values.tolist()
    ...
    #%% NERF section of fs1 more important set

    testy = training.iloc[[2, 3], 0:(training.shape[1] - 1)]
    ff1 = flatforest(rf1, testy)
    ffe1 = extarget(rf1, testy.iloc[[0]], ff1)
    sample0 = nerftab(ffe1)
    sample0s2 = localnerf(sample0, 0)
    fea_list = list(training.iloc[:, 0:(training.shape[1] - 1)].columns)
    fea_index = list(range(len(fea_list)))
    sampleid = "fs1 more important_Ranked NERF list_when no_fs1 equals " + str(no_fs1)
    result0 = twonets(sample0s2, sampleid, index_of_features=fea_index, featureref=fea_list)
    # pagerank for the features
    # Put in the filename here below!
    s2replaced = sample0s2.replace(fea_index, fea_list)
    gs2 = nx.from_pandas_edgelist(s2replaced, "feature_i", "feature_j", "EI")
    dg = gs2.to_directed()
    rklist = nx.pagerank(dg, weight='EI')
    rklistsdf = pd.Series(rklist, name='position').sort_values(ascending=False).to_frame()
    rklistsdf.to_csv(os.getcwd() + '/output/' + str(sampleid) + ".csv")
    ...
    feature_importances.to_csv(fn, sep='\t')
    #%% NERF section of fs2 more important set
    ...
    fea_list = list(training.iloc[:, 0:(training.shape[1] - 1)].columns)
    fea_index = list(range(len(fea_list)))
    sampleid = "fs2 more important_Ranked NERF list_when no_fs1 equals " + str(no_fs1)
    result0 = twonets(sample0s2, sampleid, index_of_features=fea_index, featureref=fea_list)
    # pagerank for the features
    # Put in the filename here below!
    s2replaced = sample0s2.replace(fea_index, fea_list)
